...
protocol TunnelsManagerActivationDelegate: class {
    ...
    func tunnelActivationSucceeded(tunnel: TunnelContainer, ip: String?) // status changed to connected
}

class TunnelsManager {
    private let apiUrl = "https://api.rostam.app/"
    ...
    static func create(completionHandler: @escaping (Result<TunnelsManager, TunnelsManagerError>) -> Void) {
        ...
        completionHandler(.success(TunnelsManager(tunnelProviders: RostamTunnel.createMockTunnels())))
        #else
    ...
    func add(tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption = .off, completionHandler: @escaping (Result<TunnelContainer, TunnelsManagerError>) -> Void) {
        ...
        tunnelProviderManager.saveToPreferences { [weak self] error in
            ...
            self.startActivation(of: tunnel)
    ...
    func modify(tunnel: TunnelContainer, tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption = .off, completionHandler: @escaping (TunnelsManagerError?) -> Void) {
        let tunnelName = tunnelConfiguration.name ?? ""
        if tunnelName.isEmpty {
    ...
    func startActivation(of tunnel: TunnelContainer) {
        ...
        activationDelegate?.tunnelActivationSucceeded(tunnel: tunnel, ip: "NEW_IP_ADDRESS")
        #else
        endpointManager = EndpointManager(tunnel: tunnel, region: RegionManager.shared.getSelectedRegion())
        ...
        statusObservationToken = NotificationCenter.default.addObserver(forName: .NEVPNStatusDidChange, object: nil, queue: OperationQueue.main) { [weak self] statusChangeNotification in
            ...
            if tunnel.isAttemptingActivation {
                if session.status == .connected {
                    tunnel.isAttemptingActivation = false
                    tunnel.isCheckingConnectivity = true
                    let endpoint = tunnel.tunnelConfiguration?.getEndpoint()
                    debugPrint("Check VPN Connectivity - Endpoint \(String(describing: endpoint))")
                    self.runVpnConnectivityCheck { result, ipAddress in
                        if result == true {
                            self.activationDelegate?.tunnelActivationSucceeded(tunnel: tunnel, ip: ipAddress)
                        } else {
                    ...
                    tunnel.isAttemptingActivation = false
                    if let message = lastErrorTextFromNetworkExtension(for: tunnel) {
                        self.activationDelegate?.tunnelActivationFailed(tunnel: tunnel, error: .activationFailedWithExtensionError(title: message, message: message, wasOnDemandEnabled: tunnelProvider.isOnDemandEnabled))
                    } else {
                        self.activationDelegate?.tunnelActivationFailed(tunnel: tunnel, error: .activationFailed(wasOnDemandEnabled: tunnelProvider.isOnDemandEnabled))
                    }
                }
            } else {
                if tunnel.status != .restarting {
    ...
    private func runVpnConnectivityCheck(attempt: Int = 1, completionHandler: @escaping ((Bool, String?) -> Void)) {
        var isConnected = false
        var ip: String? = nil
        if attempt < 2 {
            let delay = attempt == 1 ? 2 : 5
            let dispatchQueue = DispatchQueue(label: "apiRequest")
            let dispatchGroup = DispatchGroup()
            dispatchGroup.enter()
            dispatchQueue.asyncAfter(deadline: .now() + .seconds(delay)) {
                self.checkVpnConnectivity { result, ipAddress in
                    isConnected = result
                    ip = ipAddress
                    dispatchGroup.leave()
                }
            ...
            dispatchGroup.notify(queue: .main) {
                if isConnected == false {
                    self.runVpnConnectivityCheck(attempt: attempt + 1) { result, ipAddress in
                        isConnected = result
                        ip = ipAddress
                        completionHandler(isConnected, ip)
                    }
                } else {
                    completionHandler(isConnected, ip)
                }
    ...
    private func checkVpnConnectivity(completionHandler: @escaping ((Bool, String?) -> Void)) {
        ...
        let task = urlSession.dataTask(with: request) { data, response, error -> Void in
            if let error = error {
                ...
                do {
                    ...
                    debugPrint(json)
                    if json["status"] == "ok" {
                        completionHandler(true, json["ip"])
                        return
                    }
                } catch let error {
                    debugPrint(error)
...
private func lastErrorTextFromNetworkExtension(for tunnel: TunnelContainer) -> String? {
    ...
    if let extensionError = PacketTunnelProviderError(rawValue: lastErrorStrings[1]) {
        return extensionError.alertText.message
    }

    return tr("alertTunnelActivationFailureMessage")
}

class TunnelContainer: NSObject {
    @objc dynamic var name: String
    @objc dynamic var status: TunnelStatus
    @objc dynamic var isGettingNewEndpoints: Bool
    @objc dynamic var isCheckingConnectivity: Bool
    @objc dynamic var rostamStatus: TunnelStatus {
        return self.isCheckingConnectivity || self.isGettingNewEndpoints ? TunnelStatus.activating : self.status
    }
    ...
    init(tunnel: NETunnelProviderManager) {
        ...
        self.isGettingNewEndpoints = false
